...
int main() {
    ...
    // case 1
    // tasks run in parallel, take ~45 milliseconds
    ...
    // case 2
    // tasks run sequentially, take ~126 milliseconds
    int nLoops = 1e7 , nThreads = 4;
    for (int i = 0; i < nThreads; ++i) {
    ...
    // case 3
    // tasks run in parallel, take ~3 milliseconds
    int nLoops = 10, nThreads = 5;
    for (int i = 0; i < nThreads; ++i) {
        futures.emplace_back(std::async(std::launch::async, workerFunction, nLoops));
    }
    ...
    // case 4
    // tasks run sequentially, take ~0.01 milliseconds
    int nLoops = 10, nThreads = 5;
    for (int i = 0; i < nThreads; ++i) {
        futures.emplace_back(std::async(std::launch::deferred, workerFunction, nLoops));
    }
    // we see from above that
    // not in all cases, run parallely faster than sequentially
